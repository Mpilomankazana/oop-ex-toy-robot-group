...
public class World {
    ...
    /**
     * Creates a World loaded from a config.properties file. Reads world size,
     * visibility, and obstacle positions from the file.
     *
     ...
     * Supports an optional width, height, and type per obstacle. Format:
     * x,y[,width,height[,type]] — type defaults to MOUNTAIN, width/height
    ...
    private void loadObstacles(java.util.Properties config) {
        ...
        for (int i = 1; i <= count; i++) {
            ...
            String type = parts.length > 4 ? parts[4].trim().toUpperCase() : "MOUNTAIN";
    ...
    /**
     * Adds a single-point (1x1) Mountain obstacle at the given coordinate. Kept
    ...
    /**
     * Adds an already-constructed Obstacle (Mountain, Lake, or Pit) to the
     * world.
     *
    ...
    /**
     * Returns the robot at the given position, or null if the position is
     * empty.
     *
     * @param x x-coordinate to check
     * @param y y-coordinate to check
